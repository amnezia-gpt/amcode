...
fn tmux_split_preserves_fresh_session_composer_row_after_resize_reflow() -> Result<()> {
    ...
    let start_output = checked_output(
        Command::new("tmux")
            ...
            .arg("--")
            .arg("env")
            .arg(format!("AMCODE_HOME={}", codex_home.path().display()))
...
fn codex_binary(repo_root: &Path) -> Result<PathBuf> {
    if let Ok(path) = codex_utils_cargo_bin::cargo_bin("amcode") {
        return Ok(path);
    }

    let fallback = repo_root.join("codex-rs/target/debug/amcode");
    anyhow::ensure!(
        fallback.is_file(),
        "amcode binary is unavailable; run `cargo build -p codex-cli --bin amcode` first"
    );
    Ok(fallback)
}
