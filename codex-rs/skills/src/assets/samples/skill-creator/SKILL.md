...
- "Where should I create this skill? If you do not have a preference, I will place it in `$AMCODE_HOME/skills` (or `~/.amcode/skills` when `AMCODE_HOME` is unset) so Codex can discover it automatically."
...
Before running `init_skill.py`, ask where the user wants the skill created. If they do not specify a location, default to `$AMCODE_HOME/skills`; when `AMCODE_HOME` is unset, fall back to `~/.amcode/skills` so the skill is auto-discovered.
...
```bash
scripts/init_skill.py my-skill --path "${AMCODE_HOME:-$HOME/.codex}/skills"
scripts/init_skill.py my-skill --path "${AMCODE_HOME:-$HOME/.codex}/skills" --resources scripts,references
