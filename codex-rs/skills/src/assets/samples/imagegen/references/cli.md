...
Set a stable path to the skill CLI (default `AMCODE_HOME` is `~/.amcode`):

```
export AMCODE_HOME="${AMCODE_HOME:-$HOME/.codex}"
export IMAGE_GEN="$AMCODE_HOME/skills/.system/imagegen/scripts/image_gen.py"
```
...
- Built-in-first transparent image workflow: `SKILL.md` and `$AMCODE_HOME/skills/.system/imagegen/scripts/remove_chroma_key.py`
