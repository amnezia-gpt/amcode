...
async fn responses_mode_stream_cli() {
    ...
    let bin = codex_utils_cargo_bin::cargo_bin("amcode").unwrap();
...
async fn integration_creates_and_checks_session_file() -> anyhow::Result<()> {
    ...
    let bin2 = codex_utils_cargo_bin::cargo_bin("amcode").unwrap();
    ...
    cmd2.arg("exec")
        ...
        .arg("resume")
        .arg("--last");
    cmd2.env("AMCODE_HOME", home.path())
        .env("OPENAI_API_KEY", "dummy")
        .env("CODEX_RS_SSE_FIXTURE", &fixture);
