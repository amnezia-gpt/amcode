...
//!
//! The history is stored at `~/.amcode/history.jsonl` with **one JSON object per
...
/// Filename that stores the message history inside `~/.amcode`.
...
pub async fn append_entry(text: &str, conversation_id: &ThreadId, config: &Config) -> Result<()> {
    ...
    // Resolve `~/.amcode/history.jsonl` and ensure the parent directory exists.
