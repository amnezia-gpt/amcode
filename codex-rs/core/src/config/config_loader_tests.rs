...
async fn ignore_user_config_keeps_empty_user_layer() -> std::io::Result<()> {
    ...
    let user_layer = layers
        .get_user_layer()
        .expect("expected a user layer even when AMCODE_HOME/config.toml is ignored");
    assert_eq!(
        user_layer.config,
...
async fn returns_empty_when_all_layers_missing() {
    ...
    let user_layer = layers
        .get_user_layer()
        .expect("expected a user layer even when AMCODE_HOME/config.toml does not exist");
    assert_eq!(
        &ConfigLayerEntry {
