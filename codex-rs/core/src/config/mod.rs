...
use codex_model_provider_info::built_in_model_providers_for_backend;
...
pub struct Config {
    ...
    /// event.
    ///
    /// Example `~/.amcode/config.toml` snippet:
    ///
    /// ```toml
    ...
    /// file: AMCODE_HOME/.credentials.json
    ...
    /// Directory containing all Codex state (defaults to `~/.amcode` but can be
    /// overridden by the `AMCODE_HOME` environment variable).
    pub codex_home: AbsolutePathBuf,

    /// Directory where Codex stores the SQLite state DB.
    pub sqlite_home: PathBuf,

    /// Directory where Codex writes log files (defaults to `$AMCODE_HOME/log`).
    pub log_dir: PathBuf,

    /// Settings that govern if and what will be written to `~/.amcode/history.jsonl`.
    pub history: History,
...
/// Patch `AMCODE_HOME/config.toml` project state to set trust level.
/// Use with caution.
...
impl Config {
    ...
    pub(crate) async fn load_config_with_layer_stack(
        ...
        let first_party_backend = cfg.first_party_backend.unwrap_or_default();
        let first_party_base_url = match first_party_backend {
            FirstPartyBackend::OpenAi => cfg.openai_base_url.clone(),
            FirstPartyBackend::AmneziaRouter => cfg.amnezia_router_base_url.clone(),
        }
        .filter(|value| !value.is_empty());

        let model_providers = merge_configured_model_providers(
            built_in_model_providers_for_backend(first_party_backend, first_party_base_url),
            cfg.model_providers,
        )
