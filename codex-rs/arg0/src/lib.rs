...
///
/// 1.  Load `.env` values from `~/.amcode/.env` before creating any threads.
...
pub fn prepend_path_entry_for_codex_aliases() -> std::io::Result<Arg0PathEntryGuard> {
    ...
    // Use a AMCODE_HOME-scoped temp root to avoid cluttering the top-level directory.
