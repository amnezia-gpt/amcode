...
pub enum AuthCredentialsStoreMode {
    #[default]
    /// Persist credentials in AMCODE_HOME/auth.json.
    File,
    /// Persist credentials in the keyring. Fail if unavailable.
    Keyring,
    /// Use keyring when available; otherwise, fall back to a file in AMCODE_HOME.
    Auto,
...
pub struct Tui {
    ...
    /// Use `/theme` in the TUI or see `$AMCODE_HOME/themes` for custom themes.
    #[serde(default)]
