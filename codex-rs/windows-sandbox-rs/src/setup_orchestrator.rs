...
fn filter_sensitive_write_roots(mut roots: Vec<PathBuf>, codex_home: &Path) -> Vec<PathBuf> {
    // Never grant capability write access to AMCODE_HOME or anything under AMCODE_HOME/.sandbox,
    // AMCODE_HOME/.sandbox-bin, or AMCODE_HOME/.sandbox-secrets. These locations contain sandbox
