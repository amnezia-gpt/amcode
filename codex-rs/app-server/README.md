...
- unix socket (`--listen unix://` or `--listen unix://PATH`): websocket connections over `$AMCODE_HOME/app-server-control/app-server-control.sock` or a custom socket path, using the standard HTTP Upgrade handshake
...
opens exactly one raw stream connection to `$AMCODE_HOME/app-server-control/app-server-control.sock`
...
- `memory/reset` — experimental; clear the current `AMCODE_HOME/memories` directory and reset persisted memory stage data in sqlite while preserving existing thread memory modes; returns `{}` on success.
